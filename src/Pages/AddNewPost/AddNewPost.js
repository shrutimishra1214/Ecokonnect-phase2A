    ...
    const renderedContentInput = () => {
      const changeCurrPhase = ({index, title}) =>{
        ...
        switch (lowerCaseString(this.state.method.value)){
          ...
          case Consts.Tweet: {
            return (<button title="Tweet your thoughts to others" onClick={() => changeCurrPhase({index: 4, title: this.state.contentType})} className="py-1 profile__btn primary__btn flex-row"><span>Write a quote</span></button>)
          }
          case Consts.Poll: {
